...
} from "@/hooks/useTemperatureConverter";
  ...
  return (
    <ConverterCard>
      <div className="text-right">
        <h2 className="text-xl font-bold text-blue-800">המרת טמפרטורה</h2>
        ...
        <div className="mt-4 flex flex-row-reverse gap-4">
          ...
          >
            {Object.entries(TEMPERATURE_UNIT_LABELS).map(([id, label]) => (
        ...
        <div className="mt-6 rounded-xl bg-blue-50 px-4 py-3 text-lg font-bold text-blue-600">
          {result} {TEMPERATURE_UNIT_LABELS[toUnit]}
        </div>
      </div>
    </ConverterCard>
  );
